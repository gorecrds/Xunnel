...
class RequestRequest(models.Model):
    ...
    def message_auto_subscribe(self, updated_fields, values=None):
        """ Handle auto subscription. Two methods for auto subscription exist:
            ...
            - tracked res.users relational fields, such as user_id fields.
              Those fields must be relation fields toward a res.users record,
              and must have the track_visilibity attribute set.
            - using subtypes parent relationship: check if the current model
              being modified has an header record (such as a project for tasks)
              whose followers can be added as followers of the current records.
              Example of structure with project and task:
            ...
            :param dict values: updated values;
        """
        super(RequestRequest, self).message_auto_subscribe(
            updated_fields, values)
        if 'author_id' in updated_fields and values.get('author_id'):
            if self.env['res.partner'].browse(values['author_id']).active:
                self.message_subscribe(
                    partner_ids=[values['author_id']], force=False)
        return True
